...
async function POST({ request }) {
	const { form, configuration, type } = await request.json();
	let info;
	try {
		if (type === "quote") {
			info = await transporter.sendMail({
				from: `"ZeCoat" <${process.env.SMTP_USER}>`,
				to: "aquijada@zecoat.com",
				subject: "New Quote",
				text: `
NEW ZECOAT INQUIRY
...
This message was submitted via the ZeCoat website contact form.
  ...
  `, [
		...
		if (type === "general") info = await transporter.sendMail({
			from: `"ZeCoat" <${process.env.SMTP_USER}>`,
			to: "aquijada@zecoat.com",
			subject: "New Message",
			text: `
